...
FABRIC_SAMPLES_HOME="$(cd "${SCRIPT_DIR}/../.." && pwd)"
...
usage() {
	cat <<EOF
Usage:
	./scripts/blockchain_run.sh [prereq|full|up|deploy|upgrade|down]
...
Optional environment variables:
	TEST_NETWORK_HOME  (default: fabric-samples/test-network)
	CHANNEL_NAME       (default: mychannel)
	CC_NAME            (default: drugtracker)
	CC_LANG            (default: javascript)
	CC_SRC_PATH        (default: asset-transfer-drug/chaincode-js)
...
check_prereqs() {
	...
	if [[ $missing -ne 0 ]]; then
		echo "Hint: run ./scripts/blockchain_run.sh prereq first"
		exit 1
	fi
}
...
deploy_chaincode() {
	(
		cd "${FABRIC_SAMPLES_HOME}/asset-transfer-drug"
		...
		TEST_NETWORK_HOME="${TEST_NETWORK_HOME}" \
			./scripts/update_code_centralized.sh
	)
}
